...
byte for byte; `--fix` rewrites it.

A `table` mirror is report only, and `compare` sets what counts as drift. A
one-line summary table is a paraphrase by nature — a good summary picks
shorter, different words, so word overlap measures style rather than
accuracy: "Selectively concentrating on one system of the Existence" and
"Finite attention applied to a scope" share no words and say the same thing.
So `paraphrase`, the default, reports only a row whose term has no node,
which is the failure that silently rots a mirror when a term is renamed or
